 ...
 *
 * Structurally v1's, with one difference that matters: the input setting is now
 * "two thumbs or one pointer" rather than "phone or keyboard", because v2 has no
 * keyboard mode at all. It still re-charts on change, since how many notes a
 * chart may hold depends on how many pointers will be reaching for them.
 */
(function () {
  'use strict';
  ...
  // Analysis is async and re-runs on every settings change. The token makes a
  // newer request win, so a slow earlier one cannot overwrite the current chart.
  var analysisToken = 0;

  function describe(notes) {
    var C = RG.config;
    var slash = 0, hold = 0, bomb = 0;
    notes.forEach(function (n) {
      if (n.type === C.SLASH) slash++;
  ...
  function prepare(buffer, label, sourceId) {
    ...
    return RG.chart.build(buffer, RG.settings.diff, function (p) {
      ...
      .then(function (notes) {
        ...
        // A built-in track already shows its name on the highlighted button; a
        // file or a recording has no button, so name it here or nothing says
        // which song is loaded.
        setStatus((RG.song.id ? '' : label + ' · ') +
                  describe(notes) + ' · 길이 ' + fmtTime(buffer.duration));
  ...
  RG.songs.builtin.forEach(function (track) {
    ...
    b.addEventListener('click', function () {
      ...
      setBar(null);
      el.playBtn.classList.add('hidden');
      // Yield to the event loop so the status paints before base64 decoding
      // blocks the thread. setTimeout, not rAF: rAF never fires in a hidden tab.
      setTimeout(function () {
        RG.songs.load(track)
  ...
  // ---------- tab audio capture ----------
  // Desktop only: getDisplayMedia does not exist on any mobile browser, so on a
  ...
  // ---------- settings ----------
  // Difficulty is not a note-count dial here — it swaps the grid and the note
  // kinds — so it re-charts, and so does the input mode.
  ...
  function paintDiffSeg() {
    Array.prototype.forEach.call(el.diffSeg.children, function (c) {
      c.classList.toggle('on', c.dataset.diff === RG.settings.diff);
    });
  }
  paintDiffSeg();
  ...
  function paintInputSeg() {
    Array.prototype.forEach.call(el.inputSeg.children, function (c) {
      c.classList.toggle('on', c.dataset.input === RG.settings.input);
    });
  }
  paintInputSeg();
  ...
  el.inputSeg.addEventListener('click', function (e) {
    var b = e.target.closest('button');
    if (!b) return;
    RG.settings.input = b.dataset.input;
    paintInputSeg();
    if (RG.song.buffer) prepare(RG.song.buffer, RG.song.label, RG.song.id);
  });
